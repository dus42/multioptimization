...
def opt_trajs(fid, map_type, is_max_fuel=False):
    ...
    c_ends = pd.read_csv(f"data_generated/opensky_centroid_ends_{map_type}.csv").query("fid==@fid")
...
for map_type in ["DN"]:
    ...
    pd.concat(flights, ignore_index=False).to_csv(f"data_generated/optimal_flights_{map_type}.csv", index=False)
